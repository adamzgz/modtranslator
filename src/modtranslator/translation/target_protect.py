"""Protect target-language words in mixed-language strings before translation.

When a string contains both English and target-language words (e.g. already-translated
terms mixed with English text), the translation backend may mangle the target-language
parts. This module replaces detected target words with placeholders Cx0, Cx1...
before translation, then restores them after.

Generalised version of spanish_protect.py — works for any supported language
(ES, FR, DE, IT, PT, RU, PL).
...
from modtranslator.translation.lang_detect import get_target_words_in_text
...
# Pattern to detect our own placeholders (for restore)
_TARGET_PH_RE = re.compile(r"Cx\d+")


def protect_target_words(text: str, lang: str) -> tuple[str, dict[str, str]]:
    """Replace target-language words in text with Cx0, Cx1... placeholders.
    ...
    """
    target_words = get_target_words_in_text(text, lang)
    if not target_words:
        return text, {}
    ...
    for word, start, end in target_words:
...
def protect_target_batch(
    texts: list[str],
    lang: str,
) -> tuple[list[str], list[dict[str, str]]]:
    """Protect target-language words in multiple texts.

    Each text gets independent mappings.
    Returns (protected_texts, mappings) where each mapping is per-text.
    """
    protected_texts: list[str] = []
    mappings: list[dict[str, str]] = []
    for t in texts:
        protected, mapping = protect_target_words(t, lang)
        protected_texts.append(protected)
        mappings.append(mapping)
    return protected_texts, mappings


def restore_target_words(text: str, mapping: dict[str, str]) -> str:
    """Restore Cx0 placeholders with original target-language words.
...
def restore_target_batch(
    texts: list[str], mappings: list[dict[str, str]]
) -> list[str]:
    """Restore target-language word placeholders in multiple texts."""
    return [restore_target_words(t, m) for t, m in zip(texts, mappings, strict=True)]
